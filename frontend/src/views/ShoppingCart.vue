    ...
    <div class="content" v-if="getShoppingCart.length>0">
      <ul>
        ...
        <li class="product-list" v-for="(item,index) in getShoppingCart" :key="item.id">
          ...
          <div class="pro-img">
            <router-link :to="{ path: '/goods/details', query: {productID: getItemProductId(item)} }">
              <img :src="getImageUrl(item)" />
            </router-link>
          </div>
          <div class="pro-name">
            <router-link
              :to="{ path: '/goods/details', query: {productID: getItemProductId(item)} }"
            >{{ getItemProductName(item) }}</router-link>
          </div>
          <div class="pro-price">{{ getItemPrice(item) }}元</div>
          <div class="pro-num">
            <el-input-number
              size="small"
              :value="getItemNum(item)"
              @change="handleChange($event,index, getItemProductId(item))"
              :min="1"
              :max="getItemMaxNum(item)"
            ></el-input-number>
          </div>
          <div class="pro-total pro-total-in">{{ getItemPrice(item) * getItemNum(item) }}元</div>
            ...
            <el-popover placement="right">
              ...
              <div style="text-align: right; margin: 10px 0 0">
                <el-button
                  type="primary"
                  size="mini"
                  @click="deleteItem($event, item.id, getItemProductId(item))"
                >确定</el-button>
              </div>
      ...
      <div class="cart-bar">
        <div class="cart-bar-left">
          ...
          <span class="cart-total">
            共
            <span class="cart-total-num">{{getCartNum}}</span> 件商品，已选择
...
export default {
  ...
  methods: {
    ...
    getItemProductName(item) {
      return item.productName || item.product_name || '商品';
    },
    ...
    getImageUrl(item) {
      // 尝试多种字段名
      const imgUrl = item.productImg || item.product_picture || item.image_url;
      
      if (!imgUrl) {
        return this.getLocalBookImage(this.getItemProductId(item));
      }
      
      // 如果是本地图书路径
      if (imgUrl.startsWith('/books/')) {
        return imgUrl;
      }
      ...
      if (imgUrl.startsWith('http://') || imgUrl.startsWith('https://')) {
        // 检查是否是不可访问的外部URL
        if (imgUrl.includes('placeholder') || imgUrl.includes('via.placeholder')) {
          return this.getLocalBookImage(this.getItemProductId(item));
        }
        return imgUrl;
      }
  ...
  computed: {
    ...
    // 计算购物车商品总数
    getCartNum() {
      return this.getShoppingCart.reduce((total, item) => {
        return total + (item.num || item.product_num || 1);
      }, 0);
    },
    isAllCheck: {
      get() {
