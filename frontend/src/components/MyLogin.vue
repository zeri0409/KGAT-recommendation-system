  ...
  methods: {
    ...
    Login() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        //如果通过校验开始登录
        if (valid) {
          // KGAT后端使用OAuth2格式，需要application/x-www-form-urlencoded
          ...
          this.$axios
            ...
            .then(res => {
              // KGAT后端返回格式: { access_token: "...", token_type: "bearer" }
              if (res.data.access_token) {
                ...
                this.$axios.get("/api/auth/me", {
                  ...
                  this.isLogin = false;
                  // 用户信息存到本地
                  let user = JSON.stringify(userRes.data);
                ...
                this.$refs["ruleForm"].resetFields();
                // 弹出通知框提示登录失败信息
                this.notifyError("登录失败，请检查用户名和密码");
              }
            })
            .catch(err => {
              // 清空输入框的校验状态
              ...
              const msg = data.detail || "登录失败，请检查用户名和密码";
